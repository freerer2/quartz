...
자연 순서를 구현하는 정수 비교기를 만들려면 어떻게 해야 할까요? 이 튜토리얼의 시작 부분에서 보았던 방법을 사용하면 됩니다:
...
여전히 `toLength` 함수의 반환 타입에는 비교 가능해야 한다는 제약 조건이 있습니다. 여기에서는 항상 정수를 자연 순서로 비교할 수 있기 때문에 잘 작동하지만, 이 점을 염두에 두어야 합니다.
...
## 자연 순서(오름차순)를 사용하여 비교 가능한 객체 비교하기
